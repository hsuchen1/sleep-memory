...
// Simple heuristic for part of speech based on meaning
function guessPos(meaning) {
  if (meaning.startsWith('a ') || meaning.startsWith('an ') || meaning.startsWith('the ') || meaning.startsWith('someone ') || meaning.startsWith('something ') || meaning.startsWith('state of ') || meaning.startsWith('act of ')) return 'n';
  if (meaning.startsWith('to ') || meaning.match(/^[a-z]+s\b/) || meaning.startsWith('make ') || meaning.startsWith('cause ') || meaning.startsWith('give ') || meaning.startsWith('show ')) return 'v';
  if (meaning.endsWith('ly')) return 'adv';
  return 'adj'; // default
}
...
for (let i = 0; i < words.length; i += 20) {
  ...
  output += `  ${setNum}: [\n`;
  chunk.forEach((w, idx) => {
    output += `    { word: "${w.word}", meaning: "${w.meaning.replace(/"/g, '\\"')}", pos: "${w.pos}" }${idx < chunk.length - 1 ? ',' : ''}\n`;
  });
...
console.log('Done generating wordSets.ts');
